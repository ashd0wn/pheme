...
#
FROM ghcr.io/pheme/pheme.com:builtin AS docs

#
# Icecast-KH with Pheme customizations build step
#
FROM ghcr.io/pheme/icecast-kh-ac:latest AS icecast
...
# Add built-in docs
COPY --from=docs --chown=pheme:pheme /dist /var/pheme/docs
...
RUN touch /var/pheme/.docker

USER root

VOLUME "/var/pheme/stations"
VOLUME "/var/pheme/backups"
VOLUME "/var/lib/mysql"
VOLUME "/var/pheme/storage/uploads"
VOLUME "/var/pheme/storage/shoutcast2"
VOLUME "/var/pheme/storage/stereo_tool"
VOLUME "/var/pheme/storage/geoip"
VOLUME "/var/pheme/storage/sftpgo"
VOLUME "/var/pheme/storage/acme"
...
ENV LANG="en_US.UTF-8" \
    PATH="${PATH}:/var/pheme/storage/shoutcast2" \
    APPLICATION_ENV="production" \
    MYSQL_HOST="localhost" \
    MYSQL_PORT=3306 \
    MYSQL_USER="pheme" \
    MYSQL_PASSWORD="azur4c457" \
    MYSQL_DATABASE="pheme" \
    ENABLE_REDIS="true" \
...
WORKDIR /var/pheme/www

COPY --chown=pheme:pheme . .

RUN composer install --no-ansi --no-interaction \
    && composer clear-cache

WORKDIR /var/pheme/www/frontend
