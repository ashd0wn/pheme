...
# Get the current release channel for Pheme
get-release-channel() {
  local PHEME_VERSION="latest"
  if [[ -f .env ]]; then
    .env --file .env get PHEME_VERSION
    PHEME_VERSION="${REPLY:-latest}"
  fi
...
#
# Configure the ports used by Pheme.
#
setup-ports() {
  envfile-set "PHEME_HTTP_PORT" "80" "Port to use for HTTP connections"
  envfile-set "PHEME_HTTPS_PORT" "443" "Port to use for HTTPS connections"
  envfile-set "PHEME_SFTP_PORT" "2022" "Port to use for SFTP connections"
}
...
setup-release() {
  if [[ ! -f .env ]]; then
    curl -fsSL https://raw.githubusercontent.com/Pheme/Pheme/main/sample.env -o .env
  fi
  ...
  .env --file .env set PHEME_VERSION=${PHEME_VERSION}

  if [[ $PHEME_VERSION != $OLD_RELEASE_CHANNEL ]]; then
...
check-install-requirements() {
  ...
  echo "Checking installation requirements for Pheme..."
  ...
  if [[ $CURRENT_OS == "Linux" ]]; then
    ...
    echo "       Automated Pheme installation is not currently supported on this"
  ...
  if [[ $SCRIPT_DIR == "/var/pheme" ]]; then
    echo -en "\e[32m[PASS]\e[0m Installation Directory\n"
  else
    echo -en "\e[93m[WARN]\e[0m Installation Directory\n"
    echo "       Pheme is not installed in /var/pheme, as is recommended"
    echo "       for most installations. This will not prevent Pheme from"
...
run-installer() {
  local PHEME_RELEASE_BRANCH
  PHEME_RELEASE_BRANCH=$(get-release-branch-name)

  if [[ ! -f .env ]]; then
    curl -fsSL https://raw.githubusercontent.com/Pheme/Pheme/$PHEME_RELEASE_BRANCH/sample.env -o .env
  fi
  if [[ ! -f pheme.env ]]; then
    curl -fsSL https://raw.githubusercontent.com/Pheme/Pheme/$PHEME_RELEASE_BRANCH/pheme.sample.env -o pheme.env
  fi
  if [[ ! -f docker-compose.yml ]]; then
    curl -fsSL https://raw.githubusercontent.com/Pheme/Pheme/$PHEME_RELEASE_BRANCH/docker-compose.sample.yml -o docker-compose.yml
  fi
  ...
  curl -fsSL https://raw.githubusercontent.com/Pheme/Pheme/$PHEME_RELEASE_BRANCH/docker-compose.installer.yml -o docker-compose.installer.yml

  dc -p pheme_installer -f docker-compose.installer.yml pull
  dc -p pheme_installer -f docker-compose.installer.yml run --rm installer install "$@"

  rm docker-compose.installer.yml
}

#
# Run the initial installer of Docker and Pheme.
...
install() {
  ...
  if [[ $EUID -ne 0 ]]; then
    .env --file .env set PHEME_PUID="$(id -u)"
    .env --file .env set PHEME_PGID="$(id -g)"
  fi

  if [[ $PODMAN_MODE -ne 0 ]]; then
    .env --file .env set PHEME_PODMAN_MODE=true
  fi

  dc pull

  dc run --rm web -- pheme_install "$@"
  dc up -d
  exit
}
...
install-dev() {
  ...
  if [[ ! -f pheme.env ]]; then
    cp pheme.dev.env pheme.env

    echo "Customize pheme.env file now before continuing. Re-run this command to continue installation."
    exit
  fi
  ...
  dc build
  dc run --rm web -- pheme_dev_install "$@"
  dc up -d
  exit
}
...
update() {
  ...
  if ask "Are you ready to continue with the update?" Y; then
    ...
    curl -fsSL https://raw.githubusercontent.com/Pheme/Pheme/$PHEME_RELEASE_BRANCH/docker.sh -o docker.new.sh
    ...
    if [[ ! -s docker-compose.new.yml ]]; then
      curl -fsSL https://raw.githubusercontent.com/Pheme/Pheme/$PHEME_RELEASE_BRANCH/docker-compose.sample.yml -o docker-compose.new.yml
    fi
...
update-self() {
  ...
  curl -H 'Cache-Control: no-cache, no-store' -fsSL \
    https://raw.githubusercontent.com/Pheme/Pheme/$PHEME_RELEASE_BRANCH/docker.sh?$(date +%s) \
    -o docker.sh
  chmod a+x docker.sh
...
#
cli() {
  dc exec --user="pheme" web pheme_cli "$@"
  exit
}
...
backup() {
  ...
  dc exec --user="pheme" web pheme_cli pheme:backup "/var/pheme/backups/${BACKUP_FILENAME}" "$@"

  # Move from Docker volume to local filesystem
  d run --rm -v "pheme_backups:/backup_src" \
...
#
# Restore an Pheme backup into Docker.
# Usage:
# ./docker.sh restore [/custom/backup/dir/custombackupname.zip]
#
restore() {
  if [[ ! -f .env ]] || [[ ! -f pheme.env ]]; then
    echo "Pheme hasn't been installed yet on this server."
    echo "You should run './docker.sh install' first before restoring."
    exit 1
  fi

  if ask "Restoring will remove any existing Pheme installation data, replacing it with your backup. Continue?" Y; then
    if [[ $1 != "" ]]; then
      local BACKUP_PATH BACKUP_DIR BACKUP_FILENAME BACKUP_EXT
      BACKUP_PATH=$(readlink -f ${1:-"./backup.tar.gz"})
      BACKUP_DIR=$(dirname -- "$BACKUP_PATH")
      ...
      # Remove most Pheme volumes but preserve some essential ones.
      d volume rm -f $(d volume ls | grep 'pheme' | grep -v 'station\|install' | awk 'NR>1 {print $2}')
      d volume create pheme_backups

      # Move from local filesystem to Docker volume
      d run --rm -v "$BACKUP_DIR:/backup_src" \
        -v "pheme_backups:/backup_dest" \
        busybox mv "/backup_src/${BACKUP_FILENAME}" "/backup_dest/${BACKUP_FILENAME}"
      ...
      dc run --rm web -- pheme_restore "/var/pheme/backups/${BACKUP_FILENAME}" "$@"
      ...
      # Remove most Pheme volumes but preserve some essential ones.
      d volume rm -f $(d volume ls | grep 'pheme' | grep -v 'station\|backups\|install' | awk 'NR>1 {print $2}')

      dc run --rm web -- pheme_restore "$@"
...
restore-legacy() {
  ...
  if [ -f "$BACKUP_PATH" ]; then
    ...
    d volume rm pheme_db_data pheme_station_data
    d volume create pheme_db_data
    d volume create pheme_station_data

    d run --rm -v "$BACKUP_DIR:/backup" \
      -v pheme_db_data:/pheme/db \
      -v pheme_station_data:/pheme/stations \
...
uninstall() {
  if ask "This operation is destructive and will wipe your existing Docker containers. Continue?" N; then
    ...
    echo "All Pheme Docker containers and volumes were removed."
...
#
rollback() {
  local PHEME_ROLLBACK_VERSION
  PHEME_ROLLBACK_VERSION="$1"

  if [[ -z "$PHEME_ROLLBACK_VERSION" ]]; then
  ...
  if ask "Are you ready to continue with the rollback?" Y; then
    dc exec --user="pheme" web pheme_cli pheme:setup:rollback "${PHEME_ROLLBACK_VERSION}"
    dc down --timeout 60

    .env --file .env set PHEME_VERSION=${PHEME_ROLLBACK_VERSION}
    ...
    echo "Rollback complete. Your installation has been returned to stable version '${PHEME_ROLLBACK_VERSION}'."
...
#
up() {
  echo "Starting up Pheme services..."
  dc up -d
}

down() {
  echo "Shutting down Pheme services..."
  dc down --timeout 60
}
