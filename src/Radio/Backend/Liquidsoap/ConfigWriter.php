...
final class ConfigWriter implements EventSubscriberInterface
{
    ...
    public function writeHeaderFunctions(WriteLiquidsoapConfiguration $event): void
    {
        if (!$event->isForEditing()) {
            $event->prependLines(
                [
                    '# WARNING! This file is automatically generated by Pheme.',
        ...
        $event->appendBlock(
            <<<LIQ
            pheme_api_url = "{$stationApiUrl}"
            pheme_api_key = "{$stationApiAuth}"

            def pheme_api_call(~timeout=2.0, url, payload) =
                full_url = "#{pheme_api_url}/#{url}"
                ...
                try
                    response = http.post(full_url,
                        headers=[
                            ...
                            ("User-Agent", "Liquidsoap Pheme"),
                            ("X-Liquidsoap-Api-Key", "#{pheme_api_key}")
                        ],
                        timeout=timeout,
                        data=payload
                    )
        ...
        if ($mediaStorageLocation->isLocal()) {
            ...
            $event->appendBlock(
                ...
                def pheme_media_protocol(~rlog=_,~maxtime=_,arg) =
                    ["#{station_media_dir}/#{arg}"]
                end

                protocol.add(
                    "media",
                    pheme_media_protocol,
                    doc="Pull files from Pheme media directory.",
                    syntax="media:uri"
                )
                LIQ
            );
            ...
            $event->appendBlock(
                <<<LIQ
                def pheme_media_protocol(~rlog=_,~maxtime,arg) =
                    timeout = 1000.0 * (maxtime - time())

                    j = json()
                    j.add("uri", arg)

                    [pheme_api_call(timeout=timeout, "cp", json.stringify(j))]
                end
    ...
    public function writePreBroadcastConfiguration(WriteLiquidsoapConfiguration $event): void
    {
        ...
        $event->appendBlock(
            <<<LIQ
            # Send metadata changes back to Pheme
            last_title = ref("")
            last_artist = ref("")
