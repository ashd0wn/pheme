...
final class LogsAction
{
    ...
    private function getGlobalLogs(): array
    {
        ...
        $logPaths['pheme_log'] = [
            'name' => __('Pheme Application Log'),
            'path' => $tempDir . '/app-' . gmdate('Y-m-d') . '.log',
            'tail' => true,
        ];

        $logPaths['pheme_nowplaying_log'] = [
            'name' => __('Pheme Now Playing Log'),
            'path' => $tempDir . '/app_nowplaying-' . gmdate('Y-m-d') . '.log',
            'tail' => true,
        ];

        $logPaths['pheme_sync_log'] = [
            'name' => __('Pheme Synchronized Task Log'),
            'path' => $tempDir . '/app_sync-' . gmdate('Y-m-d') . '.log',
            'tail' => true,
        ];

        $logPaths['pheme_worker_log'] = [
            'name' => __('Pheme Queue Worker Log'),
