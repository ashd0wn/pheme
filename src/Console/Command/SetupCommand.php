...
#[AsCommand(
    name: 'pheme:setup',
    description: 'Run all general Pheme setup steps.',
)]
final class SetupCommand extends CommandAbstract
{
    ...
    public function __construct(
        private readonly PhemeCentral $acCentral,
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        // Header display
        if ($isInit) {
            $io->title(__('Pheme Initializing...'));
        } else {
            $io->title(__('Pheme Setup'));
            $io->writeln(
                __('Welcome to Pheme. Please wait while some key dependencies of Pheme are set up...')
            );
            $io->newLine();
        }
        ...
        if ($loadFixtures) {
            ...
            $this->runCommand($output, 'pheme:setup:fixtures');
            $io->newLine();
        }

        $io->section(__('Refreshing All Stations'));

        $this->runCommand($output, 'pheme:station-queues:clear');
        ...
        if ($update) {
            $io->success(
                [
                    __('Pheme is now updated to the latest version!'),
                ]
            );
        } else {
            ...
            $io->success(
                [
                    __('Pheme installation complete!'),
                    sprintf(
