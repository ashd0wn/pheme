...
- You can now specify a path when using the `pheme:media:reprocess` command to only mark items starting with the
...
- Our High-Performance Now Playing updates through Centrifugo now send the current Now Playing data immediately upon
  ...
  initial data. See
  our [updated code samples](https://www.pheme.com/docs/developers/now-playing-data/#high-performance-updates) for
  more information.
...
# Pheme 0.19.2 (Nov 7, 2023)

## New Features/Changes

- **Built-in Documentation**: Pheme now bundles its own documentation alongside the application itself. By
...
- If using S3-style storage locations, you can now indicate that Pheme should use path-style endpoints instead of
...
- We have changed the mapping for many persistent items in our Docker filesystem to map to folders
  within `/var/pheme/storage`. To preserve back-compatibility, for most users we will continue to map the individual
  folders within that as separate volumes, but if you manually manage your volume mounts, you can now just mount a
  volume at `/var/pheme/storage` instead of each of the separate subfolders.
...
- **Initial Multi-Server Support**: Pheme now includes initial support for connecting multiple Pheme servers to
...
# Pheme 0.18.3 (Jun 5, 2023)

This release solely exists to bump the version of Pheme up to 0.18.3, which was erroneously listed as the stable
release where the bug reports in 0.18.2 were fixed.

---

# Pheme 0.18.2 (Jun 5, 2023)
...
- **Fix for [CVE-2023-2191](https://nvd.nist.gov/vuln/detail/CVE-2023-21910)**: An issue was identified where a user who
  already had an Pheme account could update their display name to inject malicious JavaScript into the header menu
  ...
  of the public cannot exploit this vulnerability in an Pheme installation, so it is primarily of concern for
...
- **Master_me and Post-Processing Tweaks**: We now have built-in support
  ...
  include it in every Pheme installation. You can now also customize whether our post-processing step includes your
...
- **Install Custom Packages at Startup**: If you want to take advantage of specific Ubuntu packages available
  via `apt-get install`, you can now specify those files in an `pheme.env` environment variable
  ...
  of plugins that were previously available, add this line to your `pheme.env` file:
...
- Our Dropbox storage location support now includes support for Dropbox's new short-lived access tokens. We include
  instructions on how to set up and use Dropbox as an Pheme storage location on the Storage Locations administration
  page.
...
- The error placeholder track (by default, "Pheme - Pheme is live!") will no longer show up in track history.

- A bug preventing station-specific branding changes from applying to embedded pages has been fixed.

---

# Pheme 0.17.7 (Jan 27, 2023)
...
- The elapsed playback time on our public player pages is now based on the Pheme server time, ensuring that even if
...
- **High-Performance Now Playing Updates are Back!** A few versions ago, we had to retire our previous
  ...
  here: https://www.pheme.com/docs/developers/now-playing-data/#high-performance-updates
...
- **Cover Art Files Support**: Many users keep the cover art for their media alongside the media in a separate image
  file. Pheme now detects image files in the same folder as your media and uses it as the default album art for that
...
# Pheme 0.17.4 (Oct 24, 2022)

## Code Quality/Technical Changes

- **Smarter database migrations:** A common source of problems with Pheme upgrades is experiencing a faulty or
...
- Note: Development has slowed down while we continue to assist SilverEagle as he continues to find a new long term
  ...
  If you wish to assist SilverEagle during these times, please review this GitHub issue
    - [#5593](https://github.com/Pheme/Pheme/issues/5593)
...
- **LetsEncrypt via the Web**: We now support configuring LetsEncrypt via the web interface. If you had previously set
  ...
  been updated to the following:
    - Full chain certificate: `/var/pheme/acme/ssl.crt`
    - Private Key: `/var/pheme/acme/ssl.key`
...
# Pheme 0.17.0 (Jun 6, 2022)

## New Features/Changes

- **HLS Support**: We now support the HTTP Live Streaming (HLS) format from directly within the Pheme web UI. Once
...
- We have updated Pheme's AutoDJ scheduler to be able to handle the "Advanced" playlist configuration options
...
- Since Pheme's services are all now accessible via `localhost`, several connections have been switched from TCP/IP
...
- Docker users can now debug Slim Application Errors by editing the `SHOW_DETAILED_ERRORS` in the `pheme.env` file,
  reports should be submitted to our [issues](https://github.com/pheme/pheme/issues) section for review by our
  team.
...
# Pheme 0.16.0 (Apr 27, 2022)

## New Features/Changes

- Pheme can now process new media files, including ScreamTracker Modules (.stm, .s3m), Module/Extended Modules
...
- **Unified Docker Container**: We have combined all of our Docker containers into a single unified container that
  ...
  at [ghcr.io/pheme/pheme](https://github.com/pheme/Pheme/pkgs/container/pheme). For most users, no
  changes will be needed when migrating to the latest version of Pheme, but if you have created
  a `docker-compose.override.yml` file, you should
  follow [our instructions](https://github.com/Pheme/Pheme/issues/5191) to update the file.
...
- Internal API requests (Icecast listener auth, Liquidsoap API calls) are now handled via a separate, dedicated "back
  channel" and won't be affected as severely by heavy traffic on Pheme from public viewers or administrators.

- When processing media, we now use a combination of two libraries (php-getid3 and ffmpeg/ffprobe) to process media and
  retrieve rich metadata; this greatly expands the types of media that we can handle in Pheme to include essentially
...
- Fixed a bug when reinstalling Pheme on top of an existing (or previous) installation.

---

# Pheme 0.15.1 (Feb 18, 2022)
...
- For Docker installations, we have removed our built-in multisite configuration in favor of a simpler default
  ...
  the [instructions in our documentation](https://www.pheme.com/docs/administration/multi-site-installation/).
...
- Updated to Liquidsoap version 2.0.3 on Ansible and Docker, this change includes some stability fixes and a patch for a
  ...
  megathread for more information [#5017](https://github.com/Pheme/Pheme/issues/5017)

## Bug Fixes

- Fixed a bug where station base directories created with relative names would end up in `/var/pheme/www/web`.
...
- **Vue Components Everywhere**: As part of our Roadmap to 1.0, we've switched a vast majority of the Pheme
...
- Changes have also been made to backend software libraries as part of our development progress:
    ...
    - Added [doctrine-entity-normalizer](https://github.com/Pheme/doctrine-entity-normalizer)
    - Added [doctrine-batch-utilities](https://github.com/Pheme/doctrine-batch-utilities)
...
- The entire Pheme codebase has been set to "strict mode" in PHP, which will strictly enforce type safety across the
...
- You can optionally disable Redis entirely, instead relying on flatfile caches for session management and other Redis
  functions by setting `ENABLE_REDIS=false` in `pheme.env`. This is not recommended for most users as Redis offers
...
- One of the biggest issues with Docker file mounting has been permissions; you can now set a custom UID/GID for the
  ...
  set `PHEME_PUID` and `PHEME_PGID` in `.env` accordingly; both default to 1000.

- All up-to-date Pheme installations will opt users out of Google's new advertisement tracking system, FLoC. Learn
...
# Pheme 0.13.0 (Jun 15, 2021)

## New Features/Changes

- **Podcast Management (Beta):** You can now upload and manage podcasts directly via the Pheme web interface. Via
...
- **Automatic Theme Selection:** If you haven't set a default theme for either your user account or the Pheme public
...
- The PHP-SPX (Simple Profiling eXtension) extension has been added and can be enabled via a new environment variable in
  `pheme.env`; this will allow very simple visual profiling of the application and its memory/CPU usage over time.
...
- Pheme can now properly read _and write_ metadata to/from Flac and Ogg Vorbis files without issue.
...
- For sequential or shuffled playlists, you can now view the internal queue that the Pheme AutoDJ uses to track its
...
- An issue with some stations crashing shortly after startup has been resolved. This was caused by a safety check we
  added to the AutoDJ to check that Pheme was up and running at the same time; however, this caused issues with
  stations that don't use the Pheme AutoDJ (i.e. stations that stream live or use remote playlists).
...
- Across all Pheme repositories, the `master` branch has been renamed to `main`.
...
# Pheme 0.12 (Jan 27, 2021)

This update introduces significant new features and fixes a number of bugs reported by the community.

## New Features/Changes

- **Remote Album Art Retrieval**: If enabled in the system settings panel, Pheme will now check remote services to
  attempt to retrieve album art if it is missing, or not provided (i.e. for live DJs). By default, this system uses the
  MusicBrainz database, which is comprehensive but can be slow; if you provide an API key for the last.fm API, Pheme
...
- **Media Manager Improvements:** Some changes have been made to the media manager to improve the user experience and
  accessibility:
    ...
    - You can now instruct Pheme to re-analyze and reprocess the selected media files in the Media Manager.
...
- Unprocessable media is now stored in a separate database table along with the date/time processed and the relevant
  ...
  processable but are processed in every 5-minute sync. Pheme will automatically re-check files marked as
...
- For more advanced setups, you can now set the following environment variables in `pheme.env` to use a third-party
  Redis service instead of the one bundled with Pheme:
    - `REDIS_HOST` (default: `redis` for Docker, `localhost` for Ansible)
    - `REDIS_PORT` (default: 6379)
    - `REDIS_DB` for the database index (default: 1)

- There is a new debug CLI command, `pheme:debug:optimize-tables`, which optimizes all tables in the MariaDB
...
- Previously Pheme's codebase had a `Settings` class and a `Settings` entity that managed two separate things, which
...
- There is a new, much friendlier animation that displays when Docker installations of Pheme are waiting for their
...
- **Removal of InfluxDB**: Despite the ever-increasing complexity of Pheme, sometimes we review our dependencies to
...
- **Songs table overhaul**: A `Songs` table has existed for the entirety of Pheme's existence as the authoritative
...
# Pheme 0.10.4 (Oct 1, 2020)

This release is being tagged to incorporate all of the feature improvements, bug fixes and other changes we've made to
Pheme over the last few months into a stable release just prior to our Hacktoberfest event. We expect the
...
- Raspberry Pi support has been restored, and Pheme's Ansible installation is once again compatible with Raspberry
...
- Fixed a bug where you could continue into Pheme without finishing setup. (#2958)
...
- We have merged the configuration from our "multi-site" setup into our core Docker Compose configuration. This means
  ...
  multiple domains alongside your Pheme installation. See issue #2855 for instructions on migrating your existing
...
- There is a new CLI command to list all accounts on an Pheme installation. For Docker users, this can be invoked
  via `./docker.sh cli pheme:account:list`.
...
# Pheme 0.10.2 (May 6, 2020)

This is a primarily bug fix oriented release that includes several important improvements. All users of Pheme are
...
- You can set a default album art URL on a per-station level; if one is not set at the station level, the system-wide
  one will be used instead (and if one is not provided there, the default Pheme album art image will be used
  instead).
...
- The Pheme homepage will now remind you if you haven't run any sort of backup (either manual or automated nightly)
...
- The "Cued" time in the upcoming song queue will now accurately reflect when Pheme expects the track to actually be
  played.
...
# Pheme 0.10.0 (Apr 26, 2020)

Pheme 0.10.0 is an incremental release with several bug fixes and improvements across the application. This version
introduces minor changes to the Pheme API (see below) and is tagged as a new minor release as a result.
...
- Pheme's AutoDJ will now build a queue of songs ahead of the current song. Instead of just seeing the "next" song,
...
- Several improvements have been made to scheduling and the Pheme AutoDJ in general (#2740, #2689, #2631). This is
...
- **Schedule View**: Now that both playlists and streamers are scheduled, you can use Pheme to produce a full
...
- **Record Live Broadcasts**: Thanks to several improvements within Pheme, we now track the start and end times for
...
- **New IP Geolocation Options**: With the changes to the MaxMind GeoLite database, we've abstracted out how we perform
  ...
  and Pheme will download and use the GeoLite database instead.

## Bug Fixes and Minor Improvements

- The `pheme_web_v2` Docker image is now directly tied to the main code repository, so rolling-release updates are
...
- The "Prefer Browser URL" and "Enable SFTP Server" settings have been tweaked so they will behave more sensibly, and
  will not interfere with portions of Pheme's web interface that make API calls.

- Several sections of Pheme (including, notably, the Duplicate Songs report and Radio Automation code) have been
...
- The Pheme AutoDJ's scheduling code has been significantly overhauled to be easier to maintain and to fix a number
...
- When previewing a music track or other finite-length file within the Pheme web interface, you can seek through the
...
- **Switched Built-in FTP Service to SFTP:** We encountered a number of issues with our previous built-in FTP service,
  ...
  modifiable by changing/adding `PHEME_SFTP_PORT` in `.env` on the host) to handle all incoming connections, is a
...
- The mini music player inside the Pheme interface is now inline with the header menu instead of accessed via a
  dropdown menu.
...
# Pheme 0.9.7 (Oct 11, 2019)

## New Features

- **New Media Manager**: The Media Manager is one of the core parts of the Pheme experience, and we want it to be as
...
- You can now customize even more of the Pheme Docker installation parameters by modifying configuration lines in
  your local [pheme.env](https://github.com/Pheme/Pheme/blob/main/pheme.sample.env#L70-L80) file.

## Bug Fixes

- We have resolved a major issue with one of our third-party libraries that causes "No valid bundles" errors to appear
  any time Pheme attempts to connect to a secure URL. This can happen even on pages that don't seemingly make any
...
- This version includes a significant number of code quality improvements under the hood. You won't notice many of these
  ...
  but they are meant to make Pheme's code much more consistent and easier to maintain, which means faster bug fixes
  and more new features!

---

# Pheme 0.9.6.5 (Aug 27, 2019)

## New Features

- This incremental minor version release adds support for Pheme's built-in FTP server on Docker installations, which
  allows you to use your Pheme credentials to log in and manage media via a high-performance FTP server.

---

# Pheme 0.9.6.2 (Aug 27, 2019)
...
- Tweaked the Pheme AutoDJ's duplicate artist/title prevention algorithm to be more forgiving of playlists that
...
- Fixed a scenario where hundreds of "zombie processes" were being created by Pheme sending notification messages to
...
- When determining your external-facing IP address (which requires pinging a service that's not located on your
  installation, one of the few times we do this), we have switched to using our own Pheme Central server for this
...
# Pheme 0.9.6 (Jul 27, 2019)

It's only been just over a month since Pheme 0.9.5.1 was released, but we've made some very significant, very
important updates to the software in that time, especially in the fields of reliability and performance.

## New Features

- **The Pheme AutoDJ is Back**: A few versions ago, we had attempted to switch entirely to using Liquidsoap for our
  ...
  Pheme AutoDJ include:
    - Being able to see the next song that will play in the queue,
    - Applying playlist changes without reloading all of Pheme, and
    - Avoiding songs playing back-to-back with either the same title _or_ the same artist using our duplicate prevention
      system.

- **Introducing AzuraRelay**: This version of Pheme has built-in support for our new "relay-in-a-box"
  software [AzuraRelay](https://github.com/Pheme/AzuraRelay). Simply drop AzuraRelay onto a server that can run
  Docker, answer a few quick questions, and it will automatically connect to Pheme, detect all stations, and relay
  them all. It will also list itself as a relay on the "host" Pheme station, and report back listener and client
  details as well.
...
- **Two New NowPlaying API Sources**: Our most powerful and comprehensive set of data is all compiled into what we call
  ...
  new [Now Playing Data APIs Guide](https://www.pheme.com/docs/developers/now-playing-data/).
...
- The currently playing song is written atomically to `/var/pheme/stations/(station_name)/nowplaying.txt`, which is
  particularly useful for our [radio-video-stream](https://github.com/Pheme/radio-video-stream) example project.
...
- If you are leaving a page with a form on it that has unsaved changes, you will be prompted to confirm leaving the
  page.
- Web servers will now more aggressively cache the hashed static assets used in Pheme, improving performance.
...
- The Highcharts library has been completely replaced across the entire application. This is largely because of
  ...
  component from early Pheme development that required updating to be a fully free and open-source stack.
...
- Fixed a number of scenarios that caused the "This station is powered by Pheme" jingle to play despite a station
...
# Pheme 0.9.5 (May 14, 2019)

Work on Pheme never stops, and with the project increasing in popularity, we've been even busier than before. A lot
...
- **API Parity**: We've done a _lot_ of work to make all of the core functionality of Pheme available via our REST
  ...
  visit `your-pheme-url/api` for API documentation specific to your installed version that you can test in-browser
...
- Fixed #1405 and other bugs related to the new Liquidsoap AutoDJ process that prevented Pheme from getting proper "
...
- If Pheme detects that you've modified your installation locally (specifically your Ansible/"Traditional"
...
- When restarting a station, the Pheme error message won't play immediately, but it will play if your station has
  nothing to play.
...
Pheme AutoDJ mode have been merged. Liquidsoap handles the scheduling and playback of all "once every x" and
otherwise scheduled playlists, and Pheme's AutoDJ handles, by default, the "general rotation" playlists. If an error
occurs with Pheme's AutoDJ, Liquidsoap will manage general rotation playlists, too.
...
We have also made improvements to how Liquidsoap is notified of playlist changes from Pheme, so these notifications
...
- **"Jingle Mode" Playlists**: You can now tell Pheme to ignore metadata updates for a given playlist, so that when
...
- **Support for Two-Factor Authentication**: Now any user on Pheme can significantly improve their security by
  ...
  the [password reset instructions](https://github.com/Pheme/Pheme/blob/main/SUPPORT.md#reset-an-account-password)
...
- The Twitter web hook now has support for rate limiting, so you can tell Pheme to only send one Tweet every few
...
- The installation method formerly known as "Traditional" has been renamed to "Ubuntu Bare-Metal" installation. The
  term "traditional" conveyed to many people that this was the "normal" or preferred way of installing Pheme, when
...
- Further improvements have been made to the process of notifying Pheme of song changes, and the station profile
...
- Several new protections have been introduced to prevent users from deleting themselves, removing critical permissions
  ...
  running `[your cli command] pheme:account:set-administrator [your-email-address]`.
...
- The `station_watcher` secondary process has been removed on Docker installations, replaced by new scripting
  ...
  support for Pheme's rich metadata even when using "Manual AutoDJ" mode or "Advanced" playlists.
...
- Starting with this release, releases are now tracked on the Composer package
  manager's [Packagist repository](https://packagist.org/packages/pheme/pheme). We will be using Packagist to
...
- **Redesigned Station Profiles and Per-Station File Quotas**: The form you see when editing your station profile has
  been divided into new tabs for easier use. If you are a global administrator of a multi-tenant Pheme instance, you
...
- **Release-Only Updates**: If you prefer to update less frequently, you can now choose to only update to the latest
  tagged release of Pheme instead of using the typical "rolling release" schedule. Both `./update.sh --release`
...
- When viewing your API documentation (`your-pheme-site/api`), the documentation will allow you to test the
...
- In some previous versions, Pheme referred to a third-party CDN (CloudFlare's CDNJS) for many of its static assets
  for performance reasons. From this version forward, those assets have been moved back to the Pheme instance
  itself. This helps to preserve the self-contained nature of Pheme and avoid unnecessary downtime caused by
...
# Pheme 0.9.0 (Oct 8, 2018)

**Important note:** Pheme is a "rolling-release" product that is updated almost daily. If you're running Pheme,
...
- **Shoutcast 2 DNAS is no longer bundled with Pheme.** While Shoutcast has been a popular offering that was bundled
  with Pheme after significant demand, it has always been non-free, proprietary software, the only component in the
  Pheme stack that is not free and open-source. With the release of Shoutcast's
  new ["freemium" pricing structure](https://www.shoutcast.com/Pricing) has also come a new, more aggressive license
  associated with distribution of the software. As such, Pheme can no longer bundle Shoutcast 2 DNAS with new
...
- **Our support for Icecast is now even better suited for commercial radio stations.** Along with our withdrawal of
  ...
  version of Pheme, you will now see much more accurate information on your listeners, especially if you're using
...
- **The core "Now Playing" library is a standalone PHP library now.** We're always looking for new ways to give back to
  the open-source software community. Now, along with [AzuraForms](https://github.com/Pheme/azuraforms), we have
  spun off our [NowPlaying](https://github.com/Pheme/nowplaying) library to be a standalone component you can
...
- **Pheme now has early support for plugins.** Many of you have wanted to customize the internal workings of
  Pheme without needing to fork the main codebase and maintain your own copy. You can now do this with the help of
  our new plugin architecture, which lets you hook directly into the most important events that happen "under the hood"
  in Pheme without modifying the core code. Check out
  our [example plugin](https://github.com/Pheme/example-plugin) for a demonstration of what plugins can do.
...
- **Pheme has a new homepage!** Visit our simple yet informative new homepage, powered by VuePress,
  at [pheme.com](https://www.pheme.com/).
...
- When editing a station's profile, Pheme will automatically check if a port is in use by another station before
...
# Pheme 0.8.0 (Dec 15, 2017)

This release includes the cumulative development work done over the fall and winter of 2017 on the Pheme project. It
...
- **Broadcast to remote streams:** Running your AutoDJ from Pheme but broadcasting to a hosted radio server
...
- **Frontend dependencies relocated to CDNs**: Many of the frontend dependencies used by the web app have been moved and
  ...
  significantly lightened the size of the Pheme codebase.
