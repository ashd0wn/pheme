<?php return array (
  ...
  array (
    ...
    array (
      ...
      'All Days' => 'Все дни',
      'All listed domain names should point to this Pheme installation. Separate multiple domain names with commas.' => 'Все перечисленные доменные имена должны указывать на эту установку Pheme. Разделите несколько доменных имен запятыми.',
      ...
      'Pheme First-Time Setup' => 'Первоначальная Настройка Pheme',
      'Pheme Instance Name' => 'Название этого экземпляра Pheme',
      'Pheme ships with a built-in free IP geolocation database. You may prefer to use the MaxMind GeoLite service instead to achieve more accurate results. Using MaxMind GeoLite requires a license key, but once the key is provided, we will automatically keep the database updated.' => 'Pheme поставляется со встроенной бесплатной базой данных IP-геолокации. При желании, вместо неё вы можете использовать сервис геолокации MaxMind для достижения более точных результатов. Для использования MaxMind GeoLite требуется лицензионный ключ, как только ключ будет предоставлен, мы будем автоматически обновлять базу данных.',
      'Pheme Update Checks' => 'Проверка обновлений Pheme',
      'Pheme User' => 'Пользователь Pheme',
      'Pheme uses a role-based access control system. Roles are given permissions to certain sections of the site, then users are assigned into those roles.' => 'Pheme использует систему контроля доступа, основанную на ролях. Роли имеют права доступа к определенным разделам сайта, а затем пользователям назначаются эти роли.',
      'Pheme Wiki' => 'Pheme Wiki',
      'Pheme will scan the uploaded file for matches in this station\'s music library. Media should already be uploaded before running this step. You can re-run this tool as many times as needed.' => 'Pheme просканирует загруженный файл на совпадения в музыкальной библиотеке этой станции. Медиафайлы уже должны быть загружены перед выполнением этого шага. Вы можете повторно запускать этот инструмент столько раз, сколько необходимо.',
      'AzuraRelay is a standalone service that connects to your Pheme instance, automatically relays your stations via its own server, then reports the listener details back to your main instance. This page shows all currently connected instances.' => 'AzuraRelay - это автономный сервис, который подключается к вашему экземпляру Pheme, автоматически ретранслирует ваши станции через свой собственный сервер, а затем передает сведения о слушателе обратно в ваш основной экземпляр. На этой странице показаны все подключенные в данный момент экземпляры.',
      'Back' => 'Назад',
      ...
      'Customize Pheme Settings' => 'Настройка Параметров Pheme',
      ...
      'Donate to support Pheme!' => 'Пожертвуйте на поддержку Pheme!',
      'Down' => 'Вниз',
      'Download' => 'Скачать',
      ...
      'Enable to have Pheme automatically run nightly backups at the time specified.' => 'Включите, чтобы Pheme автоматически запускал ночное резервное копирование в указанное время.',
      ...
      'Enter "Pheme" as the application name. You can leave the URL fields unchanged. For "Scopes", only "write:media" and "write:statuses" are required.' => 'Введите «Pheme» в качестве имени приложения. Вы можете оставить поля URL без изменений. Для «Scopes» требуются только «write:media» и «write:statuses».',
      ...
      'Hide Pheme Branding on Public Pages' => 'Скрыть бренд Pheme на публичных страницах',
      ...
      'If a visitor is not signed in and visits the Pheme homepage, you can automatically redirect them to the URL specified here. Leave blank to redirect them to the login screen by default.' => 'Если посетитель не авторизован и посещает главную страницу Pheme, вы можете автоматически перенаправить его на указанный здесь URL. Оставьте пустым, чтобы перенаправить его на страницу входа по умолчанию.',
      ...
      'If enabled, Pheme will automatically record any live broadcasts made to this station to per-broadcast recordings.' => 'Если эта функция включена, Pheme будет автоматически записывать все трансляции прямого эфира, сделанные на этой станции, при каждом вещании.',
      'If enabled, Pheme will connect to the MusicBrainz database to attempt to find an ISRC for any files where one is missing. Disabling this may improve performance.' => 'Если этот параметр включен, Pheme будет подключаться к базе данных MusicBrainz, чтобы попытаться найти ISRC для любых файлов, где он отсутствует. Отключение этой опции может повысить производительность.',
      ...
      'If selected, this will remove the Pheme branding from public-facing pages.' => 'Если выбрано, то будет удалён бренд Pheme с публично расположенных страниц.',
      ...
      'Learn more about release channels in the Pheme docs.' => 'Узнайте больше о каналах релизов в документации Pheme.',
      'Learn More about Replaygain' => 'Узнайте больше о Replaygain',
      ...
      'Note: This should be the public-facing homepage of the radio station, not the Pheme URL. It will be included in broadcast details.' => 'Примечание. Это должна быть общедоступная домашняя страница радиостанции, а не URL-адрес Pheme. Будет включено в подробности вещания.',
      'Notes' => 'Заметки',
      'Notice' => 'Уведомление',
      'Now' => 'Сейчас',
      ...
      'Powered by Pheme' => 'Работает на Pheme',
      ...
      'Settings' => 'Настройки',
      'Setup instructions for broadcasting software are available on the Pheme wiki.' => 'Инструкции по настройке программного обеспечения для вещания доступны на Pheme Wiki.',
      'SFTP Host' => 'SFTP хост',
      ...
      'Shoutcast 2 DNAS is not free software, and its restrictive license does not allow Pheme to distribute the Shoutcast binary.' => 'Shoutcast 2 DNAS не является свободным программным обеспечением, и его ограничительная лицензия не позволяет Pheme распространять бинарный файл Shoutcast.',
      ...
      'Show new releases within your update channel on the Pheme homepage.' => 'Показывать новые релизы в вашем канале обновлений на главной странице Pheme.',
      ...
      'Stereo Tool is not free software, and its restrictive license does not allow Pheme to distribute the Stereo Tool binary.' => 'Stereo Tool не является бесплатным программным обеспечением, и его ограничительная лицензия не позволяет Pheme распространять двоичный файл Stereo Tool.',
      ...
      'The editable text boxes are areas where you can insert custom configuration code. The non-editable sections are automatically generated by Pheme.' => 'Редактируемые текстовые поля - это области, где вы можете вставить пользовательский код конфигурации. Не редактируемые разделы автоматически генерируются Pheme.',
      ...
      'This is an advanced feature and custom code is not officially supported by Pheme. You may break your station by adding custom code, but removing it should fix any issues.' => 'Пользовательский код в расширенных настройках официально не поддерживается Pheme. Вы можете сломать свою станцию, добавив пользовательский код, но его удаление должно решить все проблемы.',
      ...
      'This mode disables Pheme\'s AutoDJ management, using Liquidsoap itself to manage song playback. "next song" and some other features will not be available.' => 'Этот режим отключает АвтоДиджея Pheme и для управления воспроизведением песен используется сам Liquidsoap. "Следующая песня" и некоторые другие функции будут не доступны.',
      ...
      'This name will appear as a sub-header next to the Pheme logo, to help identify this server.' => 'Это название будет отображаться в качестве подзаголовка рядом с логотипом Pheme, чтобы помочь идентифицировать этот сервер.',
      ...
      'This queue contains the remaining tracks in the order they will be queued by the Pheme AutoDJ (if the tracks are eligible to be played).' => 'Эта очередь содержит треки в том порядке, в котором они будут поставлены в очередь АвтоДиджея Pheme (если эти треки имеют право на воспроизведение).',
      ...
      'Update' => 'Изменение',
      'Update Pheme' => 'Обновление Pheme',
      'Update Pheme via Web' => 'Обновление Pheme через веб-браузер',
      'Update Pheme? Your installation will restart.' => 'Обновить Pheme? Ваша установка будет перезапущена.',
      ...
      'Use API keys to authenticate with the Pheme API using the same permissions as your user account.' => 'Используйте ключи API для аутентификации с API Pheme, используя те же разрешения, что и ваша учетная запись пользователя.',
      ...
      'Wednesday' => 'Среда',
      'Weight' => 'Вес',
      'Welcome to Pheme!' => 'Добро пожаловать в Pheme!',
      ...
      'You can set a custom URL for this stream that Pheme will use when referring to it. Leave empty to use the default value.' => 'Вы можете задать пользовательский URL для этого потока, который Pheme будет использовать в ссылке на него. Оставьте пустым, чтобы использовать значение по умолчанию.',
      ...
      'This is a test message from Pheme. If you are receiving this message, it means your e-mail settings are configured correctly.' => 'Это тестовое сообщение от Pheme. Если вы получили это сообщение, это означает, что ваши настройки электронной почты настроены правильно.',
      'Test message sent successfully.' => 'Тестовое сообщение успешно отправлено.',
      'Pheme Application Log' => 'Pheme - Журнал',
      'Pheme Now Playing Log' => 'Журнал воспроизводимого сейчас Pheme',
      'Pheme Synchronized Task Log' => 'Журнал синхронизированных задач Pheme',
      'Pheme Queue Worker Log' => 'Журнал очереди рабочих процессов Pheme',
      ...
      'New Pheme Stable Release Available' => 'Доступен новый стабильный релиз Pheme',
      ...
      'New Pheme Rolling Release Available' => 'Доступен новый Rolling Release Pheme',
      ...
      'Pheme is free and open-source software.' => 'Pheme — бесплатное программное обеспечение с открытым исходным кодом.',
      'If you are enjoying Pheme, please consider donating to support our work. We depend on donations to build new features, fix bugs, and keep Pheme modern, accessible and free.' => 'Если вам нравится Pheme, рассмотрите возможность сделать пожертвование в поддержку нашей работы. Мы зависим от пожертвований для создания новых функций, исправления ошибок и поддержания актуальности, доступности и бесплатности Pheme.',
      'Donate to Pheme' => 'Пожертвовать в Pheme',
      ...
      'You can track the execution time and memory usage of any Pheme page or application from the profiler page.' => 'Вы можете отслеживать время выполнения и использование памяти любой страницы или приложения Pheme со страницы профилирования.',
      ...
      'This likely means that multiple Pheme instances are connecting to the same database. This instance is not the current active one, so synchronized tasks won\'t run on this server. If this is intentional, you can disregard this message.' => 'Вероятно, это означает, что несколько экземпляров Pheme подключаются к одной и той же базе данных. Этот экземпляр не является текущим активным, поэтому синхронизированные задачи не будут выполняться на этом сервере. Если это сделано намеренно, вы можете проигнорировать это сообщение.',
      ...
      'If you regularly use different URLs to access Pheme, you should enable the "Prefer Browser URL" setting.' => 'Если вы регулярно используете разные URL-адреса для доступа к Pheme, вам следует включить параметр "Предпочтительно URL-адрес в браузере".',
      ...
      'Pheme Initializing...' => 'Инициализация Pheme...',
      'Pheme Setup' => 'Настройка Pheme',
      'Welcome to Pheme. Please wait while some key dependencies of Pheme are set up...' => 'Добро пожаловать в Pheme. Пожалуйста, подождите, пока настраиваются некоторые ключевые зависимости Pheme...',
      ...
      'Pheme is now updated to the latest version!' => 'Pheme теперь обновлен до последней версии!',
      'Pheme installation complete!' => 'Установка Pheme завершена!',
      ...
      'Pheme Installer' => 'Установщик Pheme',
      'Welcome to Pheme! Complete the initial server setup by answering a few questions.' => 'Добро пожаловать в Pheme! Завершите первоначальную настройку сервера, ответив на несколько вопросов.',
      'Pheme Updater' => 'Обновление Pheme',
      'Change installation settings?' => 'Изменить параметры установки?',
      'Pheme is currently configured to listen on the following ports:' => 'В данный момент Pheme настроен для прослушивания следующих портов:',
      'HTTP Port: %d' => 'HTTP порт: %d',
      'HTTPS Port: %d' => 'HTTPS порт: %d',
      'SFTP Port: %d' => 'SFTP порт: %d',
      'Radio Ports: %s' => 'Радио порты: %s',
      'Customize ports used for Pheme?' => 'Настроить порты, используемые для Pheme?',
      'Writing configuration files...' => 'Запись файлов конфигурации...',
      'Server configuration complete!' => 'Настройка сервера завершена!',
      'This file was automatically generated by Pheme.' => 'Этот файл был автоматически создан Pheme.',
      ...
      'HTTP Port' => 'HTTP порт',
      'The main port Pheme listens to for insecure HTTP connections.' => 'Основной порт Pheme прослушивает небезопасные HTTP соединения.',
      'HTTPS Port' => 'HTTPS порт',
      'The main port Pheme listens to for secure HTTPS connections.' => 'Основной порт Pheme прослушивает безопасные HTTPS соединения.',
      'The port Pheme listens to for SFTP file management connections.' => 'Порт Pheme прослушивает соединения для управления файлами SFTP.',
      'Station Ports' => 'Порты Станции',
      'The ports Pheme should listen to for station broadcasts and incoming DJ connections.' => 'Порты, которые Pheme должен прослушивать для вещания станций и входящих подключений Диджеев.',
